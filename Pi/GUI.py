...
print("About to log an INFO message.")
    ...
    def checkBlogforEVA(self, dt):
        ...
        def on_success(req, data): #if blog data is successfully received, it is processed here
            ...
            def on_redirect2(req, result):
                log_info("Warning - Get EVA crew failure (redirect)")
                log_info(result)

            def on_failure2(req, result):
                log_info("Warning - Get EVA crew failure (url failure)")

            def on_error2(req, result):
                log_info("Warning - Get EVA crew failure (url error)")

            req2 = UrlRequest(iss_EVcrew_url, on_success2, on_redirect2, on_failure2, on_error2, timeout=1)

        def on_redirect(req, result):
            log_info("Warning - Get nasa blog failure (redirect)")

        def on_failure(req, result):
            log_info("Warning - Get nasa blog failure (url failure)")

        def on_error(req, result):
            log_info("Warning - Get nasa blog failure (url error)")
